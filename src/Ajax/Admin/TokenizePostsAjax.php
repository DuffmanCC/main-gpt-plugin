...
class TokenizePostsAjax extends AbstractActionable
{
    ...
    public function execute(): void
    {
        try {
            ...
            $postsArr = json_decode(stripslashes($postIds), true);
    ...
    protected function formatProduct(\WC_Product $productData, string $image): string
    {
        return
            ...
            "date_on_sale_from: " . ($productData->get_date_on_sale_from() ? $productData->get_date_on_sale_from()->date('Y-m-d H:i:s') : 'null') . "\n" .
            "date_on_sale_to: " . ($productData->get_date_on_sale_to() ? $productData->get_date_on_sale_to()->date('Y-m-d H:i:s') : 'null') . "\n" .
            "stock_quantity: {$productData->get_stock_quantity()}\n" .
            "stock_status: {$productData->get_stock_status()}\n" .
            ...
            "image: {$image}\n" .
            "variations_of_the_product: " . $this->getFlatVariations($productData->get_attributes()) . "\n";
    }
    ...
    protected function getFlatVariations(array $attributes): string
    {
        $variations = [];

        foreach ($attributes as $key => $value) {
            if ($value->get_variation()) {
                $variations[] = implode(', ', $value->get_options());
            }
        }
